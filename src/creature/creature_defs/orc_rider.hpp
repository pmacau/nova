...
OrcRiderDefData() {
    ...
    virtual ~OrcRiderDefData() = default;
    ...
    static const OrcRiderDefData& getInstance() {
        static OrcRiderDefData instance;
        return instance;
    }
...
protected:
    ...
    virtual void initializeRenderingInfo() override {
        renderingInfo.scale = glm::vec2(1100.f / 11, 800.f / 8);
        renderingInfo.spriteSheet.textureAssetID = TEXTURE_ASSET_ID::ORC_RIDER;
        renderingInfo.spriteSheet.sheetDimensions = glm::vec2(1100.f, 800.f);
    }

    virtual void initializePhysicsInfo() override {
        physicsInfo.scale = renderingInfo.scale  * 3.f;
    ...
    virtual void initializeAnimations() override {
        ...
        for (int col = 0; col < 9; ++col) {
            attack_right.frames.push_back({3, col});
