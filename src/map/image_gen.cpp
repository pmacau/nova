...
void create_terrain_map(GameMap& game_map) {
    ...
    debug_printf(DebugType::WORLD_INIT, "Generating terrain map\n");

    for (int row = 0; row < h; row++) {
        for (int col = 0; col < w; col++) {
            unsigned char r, g, b;
            switch (get_terrain(game_map[row][col])) {
                case WATER:
                    r = 0; g=0; b=255;
                    break;
                case SAND:
    ...
    int success = stbi_write_png(
        map_path("terrain_map.png").c_str(),
        out.w, out.h, out.channels, out.data, out.w * out.channels
    );

    if (success) debug_printf(DebugType::GAME_INIT, "Wrote terrain map file successfully.\n");
    else         debug_printf(DebugType::GAME_INIT, "Error terrain map file.\n");
